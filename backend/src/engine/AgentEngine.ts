...
## 출력 형식 — markdown 사양 v1.1

### 1. 메타데이터 (문서 최상단, YAML front-matter)

3개 항목을 정확히 이 키 이름으로 출력합니다:
...
- 작성부서, 작성일: 입력에 명시되지 않은 경우 위와 같은 placeholder 그대로 출력합니다 (사용자가 후속 단계에서 채움).
...
- 들여쓰기는 가독성용으로 자유롭게 사용합니다 (레벨은 글머리 문자만으로 결정됨).

### 4. 요약단어 (\`□\` 레벨 필수)

\`□\` 레벨 단락은 반드시 \`□ (요약단어) 본문\` 형식으로 시작합니다.
- 요약단어: 한 단어 또는 짧은 어구 (한글 1~3어절)
- 다른 레벨(섹션 헤더, \`○\`, \`-\`, \`·\`)에는 사용하지 않습니다
...
- **모든 본문 단락에 적용**합니다 — \`□\`, \`○\`, \`-\`, \`·\` 모든 층위와 \`=>\` 결론 화살표 단락 모두 (단, \`1.\` 섹션 헤더 자체는 제외).
...
본문 1~2페이지 분량으로 작성합니다. 과도한 세부 내용은 본문에 포함시키지 않습니다 (Forge가 본문 분량 초과 시 [붙임] 자동 분리하므로 \`[붙임]\` 표기 불필요).
