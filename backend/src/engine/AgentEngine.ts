...
const STAGE2_SYSTEM_PROMPT = `당신은 금융감독원 보고서 초안 작성 에이전트입니다.
...
주요 항목 순서: 배경 → 이슈 → 검토 → 계획(조치방향)

## 작성 원칙

- 분량: 본문 2페이지 이내 엄수. 세부내용은 [붙임]으로 분리
- 문체: 개조식, 간결, 핵심만
- 내용: 제공된 검토 결론의 쟁점·판단·근거를 충실히 반영. 새로운 법령 조회나 판단 추가 금지
- 보고서에 법령 조문을 인용할 때는 간략히 요약하고, 원문 전체를 넣지 마세요

## 완료 조건

초안이 완성되면 즉시 submit_for_review를 호출하세요.`;
...
export interface RunConfig {
  useMcpTools?: boolean;        // false이면 MCP 도구 없이 실행 (Stage 2 등)
  systemPrompt?: string;        // 미지정 시 SYSTEM_PROMPT(Stage 1) 사용
  initialInputOverride?: string; // 첫 메시지를 issue.input_text 대신 이 값으로 대체
}

export { STAGE2_SYSTEM_PROMPT };
