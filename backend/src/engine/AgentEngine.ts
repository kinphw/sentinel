...
const AGENT_MODE = parseAgentMode(process.env.SENTINEL_AGENT_MODE, 'live');
...
- 필요할 때만 유사한 과거 내부 검토문서를 찾아 배경과 처리 흐름을 확인하고
- 근거를 바탕으로 판단을 정리하고
...
- 법령과 법령해석이 1차 근거입니다. 내부 검토문서는 법에 없는 배경, 과거 검토 경위, 유사사례를 확인하는 보조 자료로만 사용하세요.
- 내부 검토문서는 반드시 search_fss_documents로 폴더명/파일명 후보를 먼저 좁힌 뒤, 적절한 문서만 get_fss_document로 본문을 확인하세요.
...
4. 법령해석: search_interpretation(짧은 키워드) -> get_interpretation(id)
5. 내부 문서: search_fss_documents(폴더명/파일명) -> get_fss_document(id)
...
확정된 법령 검토 결론 또는 기존 보고서 초안을 입력으로 받아 금감원 내부 보고서 형식으로 재구성하거나 보완합니다.
...
export const STAGE3_SYSTEM_PROMPT = `당신은 금융감독원 보고서 편집 에이전트입니다.
확정된 보고서 초안 또는 기존 편집본을 입력으로 받아, 내용의 법적 결론은 바꾸지 않고 서술과 구조를 다듬은 편집용 초안을 작성합니다.
새로운 법률 판단이나 법령 재조회는 하지 않습니다.

원칙:
- 원문 의미와 결론을 유지합니다.
- 사용자가 지시한 편집 요구사항이 있으면 최우선 반영합니다.
- 문장을 더 간결하게 정리하고, 개조식 구조와 헤더 표현을 균질하게 맞춥니다.
- 아직 HWP 자동화 전 단계이므로, 결과물은 HWP 반영 전의 텍스트 편집 초안입니다.

출력:
- 바로 검토 가능한 텍스트 편집본
- 필요한 경우 맨 앞에 간단한 편집 방향 1~2줄 포함 가능

완료 조건:
- 편집 초안 작성이 완료되면 즉시 submit_for_review를 호출하세요.`;
...
function parseAgentMode(value: string | undefined, fallback: 'live' | 'mock'): 'live' | 'mock' {
  if (!value) return fallback;
...
export class AgentEngine {
  ...
  constructor(
    ...
    this.client = AGENT_MODE === 'live' ? new Anthropic() : null;
  }
...
function buildMockDraft(params: {
  ...
  return (
    `[mock] Stage 3 개발용 편집 초안\n\n` +
    `편집 대상 요약:\n${compact}${feedbackSection}\n\n` +
    `편집 방향:\n` +
    `- 문단 구조와 문체를 정리하는 단계만 시뮬레이션함\n` +
    `- 실제 HWP 자동화 전에도 Stage 3 artifact 저장과 재선택이 가능해야 함\n\n` +
    `비고:\n` +
    `- 이 결과물은 운영용 문서가 아니라 개발/디버깅 확인용 mock 편집본입니다.`
  );
}
