...
│   └── document-mcp/      ← 내부 검토문서 DB MCP 서버
├── .vscode/               ← 로컬 개발용 mock/live 디버그 태스크
...
### P1 — MCP 도구 레이어 (법령 조회 + 내부 문서 조회) ★ 완료
...
| 내부 문서 MCP 서버 | `fss_document_db.documents`를 MCP 프로토콜로 노출 (폴더명/파일명 검색 → 단건 본문 조회) |
...
CLI를 웹 UI로 전환. 사용자가 이슈를 입력하고, 에이전트 진행 상황을 실시간 확인하며, confirm/feedback을 주는 인터페이스. React+Vite SPA, SSE 로그 스트리밍, Stage1~3 탭, 이전 결과 선택 후 직접 편집 입력 흐름을 포함한다. 상세 요건은 [docs/p3-frontend.md](docs/p3-frontend.md) 참조.

### P4 — 보고서 에이전트 ★ 완료

확정된 검토 결론을 입력으로 금감원 보고서 형식(배경/이슈/검토/계획, 개조식)으로 재구성하는 에이전트. STAGE2_SYSTEM_PROMPT, 커스텀 목차, Stage 2/3 재작성 입력 흐름이 구현 완료되었다. 상세 요건은 [docs/p4-report-agent.md](docs/p4-report-agent.md) 참조.
...
- 현재 완료 단계: **P4 — 보고서 에이전트 + P3 프론트엔드**
- 현재 작업 기준선: P1~P4 구현 완료. Express HTTP API(`npm run server`) + React 프론트엔드(`npm run dev`)로 웹 UI 운영 가능. CLI(`npm run dev` in backend/)도 계속 동작
- 개발/디버그 기준선: VS Code에서 mock/live 스택을 각각 실행 가능하며, 프론트 헤더에서 현재 연결 모드(`LIVE`/`MOCK`)를 배지로 확인 가능
- 현재 진행 단계: **P5 대기 (HWP 편집 모듈)**
- 다음 우선순위: few-shot 보고서 사례 보완 → P5 HWP 편집 모듈 설계
- 최신 갱신일: **2026-04-20**
...
| Phase | 상태 | 비고 |
|------|------|------|
| **P1** MCP 도구 레이어 | **완료** | lawquery MCP, 법령정보 MCP, 내부 검토문서 MCP 연결 및 기본 조회 검증 완료 |
| **P2** 검토 에이전트 엔진 | **완료** | SessionStore, ToolGateway, AgentEngine, 피드백 재진입, STAGE 1 검토 결론 에이전트 포함 |
| **P3** 프론트엔드 (오케스트레이터) | **완료** | React+Vite SPA. Stage1/2 탭, SSE 실시간 로그, confirm/feedback UI. `npm run server` + `npm run dev` |
| **P4** 보고서 에이전트 | **완료** | STAGE2_SYSTEM_PROMPT 구조·형식 수정(현황/이슈/검토의견/향후계획, 1/□/○/-/·). few-shot 예시 삽입. 목차 커스터마이징 지원 |
| **P5** 보고서 편집 모듈 (HWP) | 대기 | Windows COM 자동화 단계 |

### 최근 완료 항목

- 개발/운영 실행 구조 정리 — VS Code mock/live 스택 태스크, PM2 설정, 프런트 runtime mode 배지 추가
- `P1` 확장 완료 — 내부 검토문서용 `document-mcp` 추가. 폴더명/파일명 기반 후보 검색 후 단건 본문 조회하는 2단 조사 흐름 반영
...
- 내부 문서 MCP 운영 보강: 검색 인덱스/권한 분리/운영 전용 계정 검토
...
| [docs/p1-mcp.md](docs/p1-mcp.md) | **작성완료** | P1 - lawquery MCP 서버, 법령정보 API, 내부 검토문서 MCP 구현 상세 |
...
| [.mcp.json](.mcp.json) | **작성완료** | Sentinel이 사용하는 MCP 서버 연결 설정 (`law`, `interpretation`, `document`) |
| [.vscode/tasks.json](.vscode/tasks.json) | **작성완료** | 로컬 개발용 mock/live 스택 실행 태스크 |
| [.vscode/launch.json](.vscode/launch.json) | **작성완료** | 로컬 디버그용 백엔드 attach + 프런트 브라우저 실행 설정 |
| [ecosystem.config.cjs](ecosystem.config.cjs) | **작성완료** | PM2 운영/mock 실행 설정 |
...
3. **법적 정확성 우선.** 법령과 법령해석을 1차 근거로 사용하고, 내부 검토문서는 유사사례·배경 확인용 보조 자료로만 사용하십시오.
