...
> 판례 연동은 이 Phase에서 배제. 법령정보 MCP + 법령해석 MCP 2개를 구현한다.
...
에이전트: "이 이슈에는 전자금융거래법이 관련된다"
  ...
  → MCP 호출: search_interpretations(["지급결제대행", "중계", "정산"])
...
search_interpretations(params: {
  keywords: string[];   // 복수 키워드, AND 조건으로 처리
  type?: string;        // "유권해석" | "비조치의견서" | 생략 시 전체
  field?: string;       // 분야 필터, 생략 시 전체
...
get_interpretation_detail(id: number): InterpretationDetail
// 반환: { id, 질의요지, 회답, 이유 }
```
...
4. "지급결제대행, 중계 키워드로 해석례를 찾아줘"
   → interpretation-mcp: search_interpretations 호출 → 목록 반환 확인

5. 목록에서 id 하나 골라 "이 해석례 전문을 보여줘"
   → interpretation-mcp: get_interpretation_detail 호출 → 전문 반환 확인
```

이 3가지가 Claude Desktop에서 수동으로 동작하면 P2(에이전트 루프)로 진행.
